...
import { PhoneCall, Contact, ClipboardList, CalendarClock } from 'lucide-react';
...
/** Per-member calling report (Phase 6). Admin sees all; a member sees only their
 *  own row (the API scopes it). Counts are contacts by state, plus calls done. */
export default function PerformanceReal() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    let qs = '';
    if (range) qs = `?from=${new Date(Date.now() - Number(range) * 864e5).toISOString().slice(0, 10)}`;
